  ...
  generateExcel(): void{
    ...
    this.http.get('http://127.0.0.1:8000/api/totaljourlastyear').subscribe(
      (data1: any) => {
        ...
        const coopName = excelSheet.addRow(['Provincial Employees Credit Cooperative']);
        const address = excelSheet.addRow(['PCEDO Office, Old IBP Bldg., Rotary Lane, San Vicente, Tarlac City']);
        const reportName = excelSheet.addRow(['Statement of Financial Condition']);
        const asofNow = excelSheet.addRow(['As of ' + this.maxDate]);


        excelSheet.mergeCells(`A1:D1`);
        excelSheet.mergeCells(`A2:D2`);
        excelSheet.mergeCells(`A3:D3`);
        excelSheet.mergeCells(`A4:D4`);
        

        // Create the headers
        const employeeHeaders = ['ID', 'Name', 'Email'];

        // Add headers for employees with styling
        const employeeHeaderRow = excelSheet.addRow(employeeHeaders);
