...
export class SlAccountsComponent implements OnChanges {
  ...
  generateExcel(): void{
    ...
    const excelSheet = spreadSheet.addWorksheet('Subsidiary Ledger');
        ...
        // Create the headers
        const reportHeaders = ['','Particulars', 'Balance as of '+ this.lastYear , 'Loan amount ', 'Payment recieved '];
        ...
        this.ledgers.forEach(data =>{
          const res=['', data.result.name ? data.result.name : data.result.journal_name,
                    '', this.calculateDebit(data.result.journal_name),''];
          ...
          this.brokenledgers.forEach(data1=>{
              if(data.result.journal_name === data1.result.journal_name){
                if(data1.result.totald > 0){
                  list[1]= 'Transaction #'+data1.result.payables_id + ', '+data1.result.transactDate;
                }
                else if(data1.result.totalc > 0){
                  list[1]= 'Payment #'+data1.result.payables_id + ', '+data1.result.transactDate;
                }
          ...
          const resta=['', 'Total Amount Recieved','','', this.calculateCredit(data.result.journal_name)];
            this.pledgers.forEach(data4=>{
              if(data.result.journId === data4.result.journId){
                res[2]=data4.result.total_balance;
              }
            });
            excelSheet.addRow(resta);
          ...
          const restb=['', 'Total Balance','', data.result.total_balance,''];
            this.pledgers.forEach(data4=>{
              if(data.result.journId === data4.result.journId){
                res[2]=data4.result.total_balance;
              }
            });
            excelSheet.addRow(restb);
            const added3Row = excelSheet.getRow(excelSheet.rowCount);
            added3Row.eachCell((cell) => {
            cell.border = {
              top: { style: 'thin' },
              bottom: { style: 'thin' },
            };
